...
const AdvancedExample = () => {
  const [log, setLog] = React.useState<Array<{ value: any; id: number; highlighted?: boolean }>>([]);

  const selectNumber = (value: any) => {
    addLog(`selecting number: ${value}`);
  };
  const selectOptionType = (value: any) => {
    const v = typeof value === 'object' ? JSON.stringify(value) : value;
    addLog(`selecting type: ${v}`);
    return value !== 'Do not close';
  };
  const addLog = (value: any) => {
    setLog((prevLog) => [
      ...prevLog,
      {
        value,
  ...
  const toggleHighlight = (id: number) => {
    setLog((prevLog) =>
      prevLog.map((l) => {
        if (l.id === id) {
          return { ...l, highlighted: !l.highlighted };
        }
        return l;
      })
    );
  };
  const deleteLogItem = (id: number) => {
    setLog((prevLog) => prevLog.filter((l) => l.id !== id));
  };
