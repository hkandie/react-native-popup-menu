...
if (!React.forwardRef) {
  throw new Error(
    "This version of popup-menu requires RN 0.55+. Check our compatibility table."
  );
}
...
const _beforeClose=(menu: { name: any; }) =>{
  ...
  const hideMenu =
    (optionsRef && optionsRef.close && optionsRef.close()) ||
    Promise.resolve();
...
const _notify=(forceUpdate)=> {
  ...
  const next =
  _menuRegistry.getAll().find((menu) => menu.instance.isOpen()) ||
    NULL;
...
openedMenu = next === NULL ? undefined : next;
  ...
  if (prev.name !== next.name) {
    if (prev !== NULL && !prev.instance.isOpen()) {
      beforeSetState = () =>
      _beforeClose(prev).then(() => prev.instance.props.onClose());
    }
    if (next !== NULL) {
...
const onBackdropRef = (r) => {
  ...
  const _getOpenedMenu=()=> {
    const name =
    _placeholderRef && _placeholderRef.state.openedMenuName;
 ...
 const _ownLayout = layout;
  const _onLayout = ({ nativeEvent: { layout } }) => {
    ...
    measure(trigger).then((triggerLayout) => {
      debug(
        "got trigger measurements after context layout change",
        triggerLayout
      );
