...
function axisPositiveSideProperties({ tPos, tDim }: { tDim: number; tPos: number }) {
...
function centeringProperties({ oDim, wDim, tPos, tDim }: Options) {
 ...
 */
function axisCenteredPositionProperties(options: Options) {
...
function getCenteringPrice(options: Options) {
...
function getTopPrice(hOptions: Options, vOptions: Options) {
...
function getBottomPrice(hOptions: Options, vOptions: Options) {
    const centerOffset = getCenteringPrice(vOptions);
...
function getLeftPrice(hOptions: Options, vOptions: Options) {
...
function getRightPrice(hOptions: Options, vOptions: Options) {
    const centerOffset = getCenteringPrice(hOptions);
...
function topProperties(hOptions: Options, vOptions: Options, isRTL: boolean) {
...
function bottomProperties(hOptions: Options, vOptions: Options, isRTL: boolean) {
    const centered = axisCenteredPositionProperties(vOptions);
...
function rightProperties(hOptions: Options, vOptions: Options, isRTL: boolean) {
...
function leftProperties(hOptions: Options, vOptions: Options, isRTL: boolean) {
    const centered = axisCenteredPositionProperties(hOptions);
...
export function computeProperties(
    { windowLayout, triggerLayout, optionsLayout }: { [key: string]: any },
    placement: string,
...
interface PopoverProps {
    anchorStyle?: any;
    placement: Placement;
    preferredPlacement: Omit<Placement, 'auto'>;
...
const dynamicAnchorStyle = ({ offset, placement, isRTL }: { offset: number; placement: string; isRTL: boolean }) => {
...
const Popover = (props: PopoverProps) => {
    const scaleAnim = new Animated.Value(0.1);
    useEffect(() => {
    ...
    function close() {
        return new Promise((resolve) => {
            Animated.timing(scaleAnim, {
                duration:
                    props.closeAnimationDuration !== undefined ? props.closeAnimationDuration : CLOSE_ANIM_DURATION,
                toValue: 0,
...
const getContainerStyle = ({ placement, isRTL }: { placement: string; isRTL: boolean }) => {
    const start: { [key: string]: { flexDirection: string } } = {
        left: {
