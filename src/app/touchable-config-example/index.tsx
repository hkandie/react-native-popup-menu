...
import { TouchableOpacity, TouchableHighlight, TouchableWithoutFeedback, Button } from 'react-native';
import Menu, { MenuProvider, MenuOptions, MenuOption, MenuTrigger } from '../../molecules';

const TouchableExample = () => {
  const [state, setState] = React.useState({ Touchable: Button });

  const { Touchable } = state;
  const buttonText = 'Select ' + (Touchable ? getDisplayName(Touchable) : 'default');
  return (
    <MenuProvider style={{ flexDirection: 'column', padding: 30 }}>
      <Menu onSelect={(Touchable: any) => setState({ Touchable })}>
        <MenuTrigger
          customStyles={{
